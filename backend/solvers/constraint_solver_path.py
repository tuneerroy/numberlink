            ...
            for i in range(self.n_rows):
                for j in range(self.n_cols):
                    for p in range(max_path_length):
                        self.vars[(i, j, val, p)] = self.model.NewBoolVar(
                            f"x_{i}_{j}_{val}_{p}"
                        )
            for p in range(max_path_length):
                self.phantom_vars[(val, p)] = self.model.NewBoolVar(
                    f"phantom_{val}_{p}"
                )
        ...
        for val, (start, end) in self.pairs.items():
            ...
            for p in range(1, self.n_rows * self.n_cols):
                self.model.AddImplication(
                    self.vars[(end[0], end[1], val, p - 1)], self.phantom_vars[(val, p)]
                )
            ...
            for p in range(1, self.n_rows * self.n_cols):
                for i in range(self.n_rows):
                    for j in range(self.n_cols):
                        ...
                        self.model.Add(sum(neighbor_vars) == 1).OnlyEnforceIf(
                            current_var
                        )
            ...
            for p in range((self.n_rows * self.n_cols) - 1):
                self.model.AddImplication(
                    self.phantom_vars[(val, p)], self.phantom_vars[(val, p + 1)]
                )

            # Each position is visited at most once (2)
            for p in range(self.n_rows * self.n_cols):
                occupied_positions = [
                    self.vars[(row, col, val, p)]
